...
### Changed
- Split the OIDC client into focused configuration, token, identity, profile-photo, logout, and request-context helpers, centralize API JSON parsing and auth/session context handling for controllers, and move Teams presence sync onto a smaller Graph orchestration layer with dedicated HTTP helpers.
