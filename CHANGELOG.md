...
- Implement checklist item 16 by moving `generateContactHistoryId`, `pushContactHistory`, `addContactMoment`, and contact-history highlight reset ownership from legacy `app.js` into `assets/js/app/slices/contact-history-slice.js`, while keeping `app.js` compatibility wrappers delegating to the slice API.
...
- Implement checklist Item 18 by removing `window.kiwiGet*SliceDependencies`, wiring customer-detail/contact-history/order/delivery-remarks/app-shell dependency resolvers explicitly in `assets/js/app/index.js`, and exposing a single legacy dependency hub (`window.kiwiLegacySliceDependencies`) from `assets/js/app.js`.
...
- Update `plan/app-js-to-slices-checklist.md` domain status checkboxes to reflect current merge/implementation state (Items 1-13 complete, including the Item 13 bridge-removal cleanup), and keep explicit bridge-removal prerequisites documented for traceability.
- Implement Item 13 runtime compatibility bridging by introducing a temporary `window.kiwiRuntimeCompatibilityBridge` export in `app.js`, routing `call-agent-runtime.js` dependency calls (`addContactMoment`, `getDispositionCategories`, `selectCustomer`, `showToast`, `startCallSession`) through bridge-first/fallback resolution, and adding focused bridge tests.
- Finalize Item 13 bridge removal by wiring `call-agent-runtime.js` to explicit injected dependencies via `kiwiCallAgentRuntime.configureDependencies`, removing runtime fallback resolution through `window` globals, deleting `installRuntimeCompatibilityBridge`/`RUNTIME_COMPATIBILITY_BRIDGE_NAMESPACE` from `app.js`, and updating runtime dependency tests.
- Migrate Item 12 app-shell behavior from legacy `app.js` into `assets/js/app/slices/app-shell-slice.js`, move global `keydown`/`click`/`change` listeners into that slice, register the `close-form` action there, and keep `app.js` compatibility wrappers delegating `endSession`, `closeForm`, and `showToast` through the slice API.
- Migrate Item 11 delivery date picker/calendar engine from legacy `assets/js/delivery-date-picker.js` into `assets/js/app/slices/delivery-date-picker-slice.js`, register delivery-calendar actions in that slice, keep legacy compatibility globals (`initDeliveryDatePicker`, `navigateCalendar`, `selectDeliveryDateByString`), and stop loading the legacy delivery-date-picker script from `index.html`.
- Migrate Item 10 article catalog search/order engine from legacy `assets/js/article-search.js` into `assets/js/app/slices/article-search-slice.js`, move Item 10 action routing ownership into the slice, expose legacy compatibility globals (`initArticleSearch`, `renderOrderItems`, `getOrderData`, `orderItems`) from that slice for `app.js`, and stop loading the legacy `article-search.js` script from `index.html`.
