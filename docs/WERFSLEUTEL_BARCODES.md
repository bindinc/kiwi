...
De UI leest automatisch alle werfsleutels uit `assets/onepager_werfsleutels.md`. Iedere rij in de onepager heeft een `offerId`; daar bouwen we een barcode van met het patroon `872` + `offerId` (laatste 10 cijfers, links opgevuld met nullen). Dankzij dit patroon komen barcodes altijd overeen met de brondata en hoeven we geen aparte overrides meer te beheren.
